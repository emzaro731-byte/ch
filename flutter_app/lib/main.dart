...
class Tile extends StatelessWidget{final String symbol,price,change;const Tile({required this.symbol,required this.price,required this.change,super.key});Widget build(BuildContext c){final up=!change.startsWith('-');return Card(child:ListTile(leading:CircleAvatar(backgroundColor:const Color(0xff2b2f36),child:Text(symbol[0])),title:Text(symbol.replaceAll('USDT','/USDT'),style:const TextStyle(fontWeight:FontWeight.bold)),subtitle:const Text('24h'),trailing:Column(mainAxisAlignment:MainAxisAlignment.center,crossAxisAlignment:CrossAxisAlignment.end,children:[Text(price),Text(change,style:TextStyle(color:up?Colors.greenAccent:Colors.redAccent))])));}}
